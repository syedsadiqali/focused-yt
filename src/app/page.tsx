...
function VideoCard({
  ...
  if (compact) {
    return (
      <div
        onClick={onClick}
        className={`flex gap-2 cursor-pointer rounded-xl p-1.5 hover:bg-gray-100 transition-colors ${active ? "bg-gray-100" : ""}`}
      >
        <div className="relative shrink-0 w-40 rounded-lg overflow-hidden bg-gray-100" style={{ aspectRatio: "16/9" }}>
          {thumb?.url && (
        ...
        <div className="py-0.5 min-w-0">
          <p className="text-xs font-semibold line-clamp-3 text-gray-900 leading-snug">{video.title}</p>
          {video.channelTitle && (
  ...
  return (
    <div onClick={onClick} className="cursor-pointer group">
      ...
      <div className="mt-2 space-y-0.5">
        <p className="text-sm font-semibold line-clamp-2 text-gray-900 leading-snug">{video.title}</p>
        {video.channelTitle && (
          <p className="text-xs text-gray-500">{video.channelTitle}</p>
        )}
        <p className="text-xs text-gray-400 flex gap-1.5 flex-wrap">
      ...
      <header className="sticky top-0 z-10 bg-white border-b border-gray-200">
        <div className="flex items-center gap-4 px-4 h-14 max-w-screen-2xl mx-auto">
          ...
          >
            ...
            <span className="text-gray-900">FocusedTube</span>
          </button>
          ...
          <div className="flex flex-1 max-w-xl mx-auto">
            <input
              ...
              placeholder="Search"
              className="flex-1 px-4 py-2 text-sm border border-gray-300 border-r-0 rounded-l-full outline-none focus:border-blue-500 bg-white"
            />
            <button
              onClick={getData}
              disabled={isLoading}
              className="px-5 bg-gray-50 hover:bg-gray-100 border border-gray-300 rounded-r-full flex items-center justify-center disabled:opacity-50 transition-colors"
            >
              {isLoading
          ...
          {videos && !selectedVideo && (
            <div className="flex items-center gap-0.5 ml-auto border border-gray-200 rounded-lg p-0.5">
              ...
              <div className="flex items-center gap-1 border border-gray-200 rounded-lg p-0.5 ml-auto">
                {([["sm", "Small"], ["md", "Medium"], ["theater", "Theater"]] as [PlayerSize, string][]).map(([size, label]) => (
                  <button key={size} onClick={() => setPlayerSize(size)}
                    className={`px-3 py-1 rounded text-xs font-medium transition-colors ${playerSize === size ? "bg-gray-900 text-white" : "text-gray-500 hover:text-gray-900"}`}>
                    {label}
                  </button>
                ))}
              </div>
        ...
        <main className="max-w-screen-2xl mx-auto px-6 py-6">
          {selectedPlaylist && (
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-sm text-gray-500">Playlist videos</span>
            </div>
          )}
